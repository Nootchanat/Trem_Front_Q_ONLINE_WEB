...
const Profile = () => {
    const [userData, setUserData] = useState(null);
        ...
        useEffect(() => {
            ...
            const userId = storedUserData ? JSON.parse(storedUserData).data.user_id : null;
        
            const fetchUserData = async () => {
              try {
                const response = await axios.get(`https://puce-enchanting-salmon.cyclic.app/apis/patients/${userId}`);
